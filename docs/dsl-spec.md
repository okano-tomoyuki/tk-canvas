...
ルートは画面そのもので、**生成されるクラスはルートのクラスを継承する**（ADR 0011）。
ウィンドウ（`tk.Tk` / `tk.Toplevel`）のほか、他の画面に埋め込む部品として Frame をルートにできる。

| キー | 必須 | 型 | 説明 |
|---|---|---|---|
| `id` | ○ | 識別子 | デザイナー上でルートを指す名前。生成コードのメンバにはならない（生成クラス自身がルート） |
| `class` | ○ | `"tk.Tk"` \| `"tk.Toplevel"` \| `"ttk.Frame"` \| `"tk.Frame"` \| `"ttk.Labelframe"` \| `"tk.LabelFrame"` | 生成されるクラスの基底クラス |
| `window` | | オブジェクト | wm 系の設定（下表）。`tk.Tk` / `tk.Toplevel` のときだけ書ける |
| `options` | | 名前 → 値 | §6 |
| `layout` | | オブジェクト | §7.1 |
| `bindings` | | 配列 | §8 |
| `children` | | ウィジェットの配列 | |
...
- `tk.Tk` / `tk.Toplevel` はルート以外に使えない（Frame 類はルートにも子にも使える）。
- `window` はルートにのみ書ける。
- ルートの `options` は、基底クラスの生成後に `configure` / `config` で設定する。
  生成時にしか指定できないオプション（`class` など）は生成コードに反映されない（警告）。
  必要なら、生成したクラスのコンストラクタ引数（Python の `**kwargs`、C++ の `options`）で渡す。
...
- 生成されるクラスの基底クラスのメンバと同じ名前でない（例: `title`、`destroy`、`frame`、`text`、`id`、`post`）。
  基底クラスのメソッドや属性を上書き・隠蔽しないため（ADR 0011）。ルートのクラスを後から変えても問題が起きないよう、
  ルートにできるすべてのクラスのメンバをまとめて予約する（一覧は `packages/core/src/baseMembers.ts`）。
  ルートの `id` はメンバにならないため、この制限を受けない
...
| | `reserved-name` | エラー | 生成されるクラスの基底クラスのメンバと同じ名前 |
...
| | `window-not-allowed` | エラー | ウィンドウでない（Frame の）ルートに window を書いた |
