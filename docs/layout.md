...
- ルートがウィンドウ（Tk / Toplevel）ならタイトルバー付きの枠で描く。子もジオメトリの指定もない空のウィンドウは、Tk と同じく 200x200 で描く。
- ルートが Frame 類（部品。ADR 0011）なら、タイトルバーなしで要求サイズのまま描き、周りに点線の台（最小 240x160）をとる。
  台の上でルートの外側にドロップした場合はルートへの追加・移動とする（空のルートは大きさが 0 のため）。
...
- **表示の拡大率（ズーム）** を今後導入する予定。現在は 1 ピクセル = 1 CSS ピクセルで描いている。
